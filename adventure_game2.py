...
def begin_adventure():
    ...
    response = valid_answer("You wake up in a " +
                            random.choice(random_setting) +
                            " forest, not sure how you got there"
                            " Do you STAY or do you EXPLORE?\n",
                            "explore", "stay")
    if "stay" in response:
        print_messages("Uh oh! You've been eaten by a monster :( ")
    elif "explore" in response:
        print_messages("Wow! You've come across a pot of gold with a map home!"
                       + "Congrats :)")
